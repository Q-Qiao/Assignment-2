...
public class DeveloperAPI implements ISerializer {
    ...
    public boolean addDeveloper(Developer developer) {
        if (this.isValidDeveloper(developer.getDeveloperName())){
            return false;
        }
    ...
    public Developer getDeveloperByIndex(int index){
        if (Utilities.isValidIndex(this.developers, index)){
    ...
    public Developer getDeveloperByName (String developerName){
        int index = this.retrieveDeveloperIndex(developerName);
        if (index != -1){
            return this.developers.get(index);
        }
        return null;
    ...
    public String listDevelopers(){
        ...
        for (Developer developer : this.developers){
            listDevelopers += this.developers.indexOf(developer) + ": " + developer + "\n";
        }
    ...
    public boolean updateDeveloperWebsite(String developerName, String developerWebsite){
        if (this.isValidDeveloper(developerName)){
            Developer developerToUpdate = this.getDeveloperByName(developerName);
    ...
    public Developer removeDeveloper(String developerName){
        int index = this.retrieveDeveloperIndex(developerName);
        if (index != -1) {
            return this.developers.remove(index);
        }
        return null;
    }
    ...
    public int retrieveDeveloperIndex(String developerName){
        for (Developer developer : this.developers){
            if (developer.getDeveloperName().equalsIgnoreCase(developerName)){
                return this.developers.indexOf(developer);
            }
        }
        return -1;
    }
    ...
    public void load() throws Exception {
        ...
        this.developers = (List<Developer>) in.readObject();
        in.close();
    }
    ...
    public void save() throws Exception {
        ...
        out.writeObject(this.developers);
        out.close();
    }
